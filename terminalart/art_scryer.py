...
class SystemManager:
    ...
    def enter_fullscreen(self):
        # ESC [ 9 ; 1 t = Maximize, ESC [ 10 ; 1 t = Fullscreen
        sys.stdout.write("\033[9;1t\033[10;1t")
        sys.stdout.flush()
    def inhibit_sleep(self):
        try:
            cmd = ["dbus-send", "--print-reply", "--dest=org.freedesktop.ScreenSaver",
                   "/org/freedesktop/ScreenSaver", "org.freedesktop.ScreenSaver.Inhibit",
                   "string:lmtlss_soul_scryer", "string:Scrying Session Active"]
            res = subprocess.check_output(cmd, stderr=subprocess.DEVNULL).decode()
            if "uint32" in res:
                self.inhibit_cookie = res.split("uint32")[-1].strip()
        except: pass
    def release_sleep(self):
        if self.inhibit_cookie:
            try:
                subprocess.call(["dbus-send", "--dest=org.freedesktop.ScreenSaver",
                                "/org/freedesktop/ScreenSaver", "org.freedesktop.ScreenSaver.UnInhibit",
                                f"uint32:{self.inhibit_cookie}"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
            except: pass
import curses
import time
import math
...
def get_text_glyph(entropy_byte, entropy_val):
    ...
    weights = [1.04444 if ch == '☿' else (1.0333 if ch == '⚸' else 1.0) for ch in pool]
    total_weight = sum(weights)
    
    mix2 = ((mix >> 13) ^ (mix << 7) ^ (entropy_byte * 7919)) & 0xFFFFFFFF
    roll = (mix2 % 1000000) / 1000000.0 * total_weight
...
def is_point_on_mask_thread(u, v, scale, mask):
    ...
    # Absolute Crispness: Check exact mathematical core pixel first
    if mask_bit(mask, mx, my): return True
    
    # Dynamic Anti-Aliasing: Only expand search radius to thicken lines if 
    # the sigil is rendered small enough to risk 'jumping over' fine threads. 
    # At larger scales, keep it razor crisp.
    if scale < 45.0:
    ...
    # Check diagonals only for extremely small scales
    if scale < 22.0:
...
def refresh_verified_sigil_registry():
    ...
    if registry:
        ...
        for e in registry:
            w = 1.0
            identifier = (str(e.get("entity", "")) + " " + str(e.get("source_title", "")) + " " + str(e.get("pbm", ""))).lower()
            if e.get("is_lilith_grand"):
                w = 1.0333
            elif "prometheus" in identifier:
                w = 1.055555
